...
int main() {
	...
	while (T--) {
		...
		std::vector<int> logs(P), pw(P - 1);
		for (int i = 0, base = 1; i < P - 1; ++i, base = 1ll * base * g % P) {
			logs[base] = i;
			pw[i] = base;
		}
		std::vector<Mint> F(P);
		...
		auto Check = [&](int M) {
			...
			for (int i = 1; i <= M; ++i) G[logs[i]] = 1;
			G = F * G;
			std::vector<int> ret;
			for (int i = P - 1; i < (int)G.size(); ++i) G[i % (P - 1)] += G[i];
			for (int i = 0; i < P - 1; ++i)
				if (G[i]() >= M) ret.emplace_back(i);
			return ret;
		};
		int l = 0, r = P - 1;
		while (l < r) {
		...
		auto ans = Check(l);
		std::cout << ans.size() << ' ' << l + 1 << '\n';
		for (auto &c : ans) c = pw[c];
