...
int main() {
	...
	std::vector<std::vector<std::array<mint, 2>>> f(n + 1, std::vector<std::array<mint, 2>>(n + 1, {0, 0}));
	f[1][1][1] = 1;
	for (int i = 2; i <= n; ++i) {
		int cur = i & 1, pre = (i & 1) ^ 1;
		f[cur].assign(n + 1, {0, 0});
		for (int j = 1; j < i; ++j) {
			f[cur][j][0] += f[pre][j][0] * j + f[pre][j][1] * (a[i - 1] - 1);
			f[cur][j + 1][0] += f[pre][j][0];
			if (a[i - 1] == j + 1) f[cur][j + 1][1] += f[pre][j][1];
			else f[cur][j][1] += f[pre][j][1];
		}

	}
	mint ans = 1;
	for (int i = 1; i <= n; ++i) ans += f[n & 1][i][0];
	std::cout << ans;
	return 0;
}